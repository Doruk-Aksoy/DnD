...
Script 900 (int opt, int showInv) NET {
	// dont let player cheese the petrify with this!
...
Script "DnD Selected Dungeon Sync" (int pnum, int val) CLIENTSIDE {
	if(ConsolePlayerNumber() != pnum || GameType() == GAME_SINGLE_PLAYER)
		Terminate;
...
Script "DnD Menu Reset on Enter" ENTER {
	int pnum = PlayerNumber();
	int ch_factor = (100 + PlayerModData[pnum].vals[PSTAT_SHOPSTOCK_INCREASE]), temp, ammo_bonus = GetAmmoCapIncrease(pnum);
...
Script "DnD Menu Input Loop" (void) CLIENTSIDE {
	...
	while(CheckInventory("ShowingMenu")) {
		...
		if(curopt != curopt_prev) {
			...
			pstat_text.text = ""; // reset stat block text
		...
		if(boxid != boxid_prev) {
			if(boxid != MAINBOX_NONE && IsBoxChangeException(curopt, boxid)) {
				...
				(
					...
					PlayerCursorData.itemHovered != GetItemSyncValue(pnum, DND_SYNC_ITEMTOPLEFTBOX, boxid - 1 - PlayerCursorData.itemHoveredOffset, -1, PlayerCursorData.itemHoveredSource) - 1
				)
				{
		...
		if(PlayerCursorData.itemHovered != -1/* && (PlayerCursorData.itemClicked == -1 || CheckInventory("DnD_UsedTwoItemRequirementMaterial"))*/) {
			if(boxid == MAINBOX_NONE || PlayerCursorData.hoverNeedsReset)
				ResetCursorHoverData();
			else if(curopt != MENU_LOAD_CRAFTING_TRANSMUTING || boxid >= MATERIALBOX_OFFSET_BOXID) {
				if(PlayerCursorData.itemHoveredDim.x) {
					DrawInventoryInfo(PlayerCursorData.owner_pnum);
				}
				else {
					if(!CheckInventory("LimitedCraftDraw") || (PlayerCursorData.itemHoveredType != DND_ITEM_ORB && PlayerCursorData.itemHoveredType != DND_ITEM_TOKEN)) {
						// don't redraw if its in crafting material menu, just draw once
						DrawCraftingInventoryInfo(
					...
					if(curopt == MENU_LOAD_CRAFTING_WEAPON || curopt == MENU_LOAD_CRAFTING_INVENTORY)
						GiveInventory("LimitedCraftDraw", 1);
				}
			}
		}
		...
		if(PlayerCursorData.itemDragged != -1 && PlayerCursorData.itemDragInfo.topboxid != -1) {
		...
		if(CheckInventory("DnD_ResetDrawnStatText")) {
			pstat_text.text = "";
			TakeInventory("DnD_ResetDrawnStatText", 1);
		}
		...
		if(redraw || CheckInventory("DnD_RefreshPane")) {
			...
			if(curopt == MENU_STAT1) {
				...
				HudMessage(s:"\c[Y5]", l:"DND_STAT18", s:" \c-", d:CheckInventory("Level"), s: " \cd", l:GetClassLabel(StrParam(s:"CLASS", d:CheckInventory("DnD_Character") - 1), DND_CLASS_LABEL_NAME); HUDMSG_PLAIN, RPGMENUITEMID, CR_WHITE, 192.1, 64.0, 0.0, 0.0);
				HudMessage(s:"\c[Y5]", l:"DND_STAT16", s:": \c-", d:GetPlayerExp(pnum), s:" / ", d:LevelCurve[CheckInventory("Level") - 1]; HUDMSG_PLAIN, RPGMENUITEMID - 1, CR_WHITE, 192.1, 80.0, 0.0, 0.0);
				HudMessage(s:"\c[Y5]", l:"DND_STAT19", s:": \c-$", d:GetPlayerCredit(pnum); HUDMSG_PLAIN, RPGMENUITEMID - 2, CR_WHITE, 192.1, 96.0, 0.0, 0.0);
				HudMessage(s:"\c[Y5]", l:"DND_MENU_BUDGET", s:": \c-$", d:CheckInventory("Budget"), s:"k"; HUDMSG_PLAIN, RPGMENUITEMID - 3, CR_WHITE, 192.1, 112.0, 0.0, 0.0);
				HudMessage(s:"\c[Y5]", l:"DND_ATTRIBUTEPTS", s:": \c-", d:CheckInventory("AttributePoint"); HUDMSG_PLAIN, RPGMENUITEMID - 4, CR_WHITE, 192.1, 128.0, 0.0, 0.0);
				...
				if(!PlayerModData[pnum].vals[PSTAT_EX_UNITY]) {
					...
					HudMessage(d:STAT_BASE + CheckInventory("PSTAT_Strength"); HUDMSG_PLAIN, RPGMENUITEMID - 8, CR_WHITE, 256.1, 144.0, 0.0);
					HudMessage(d:STAT_BASE + CheckInventory("PSTAT_Dexterity"); HUDMSG_PLAIN, RPGMENUITEMID - 9, CR_WHITE, 256.1, 160.0, 0.0);
					HudMessage(d:STAT_BASE + CheckInventory("PSTAT_Intellect"); HUDMSG_PLAIN, RPGMENUITEMID - 10, CR_WHITE, 256.1, 176.0, 0.0);

					// draw bonus stat acquired
					HudMessage(s:"(\cd", d:GetStrength(pnum), s:"\c-)"; HUDMSG_PLAIN, RPGMENUITEMID - 11, CR_WHITE, 320.1, 144.0, 0.0);
					HudMessage(s:"(\cd", d:GetDexterity(pnum), s:"\c-)"; HUDMSG_PLAIN, RPGMENUITEMID - 12, CR_WHITE, 320.1, 160.0, 0.0);
					HudMessage(s:"(\cd", d:GetIntellect(pnum), s:"\c-)"; HUDMSG_PLAIN, RPGMENUITEMID - 13, CR_WHITE, 320.1, 176.0, 0.0);
				}
				else {
					DrawToggledLabel("STR", DND_NOLOOKUP, -1, boxid, MBOX_1, RPGMENUITEMID - 5, "\c[Y5]", "\c[B1]", 192.1, 144.0);
					DrawToggledLabel("DEX", DND_NOLOOKUP, -1, boxid, MBOX_2, RPGMENUITEMID - 6, "\c[Y5]", "\c[B1]", 192.1, 160.0);
					DrawToggledLabel("INT", DND_NOLOOKUP, -1, boxid, MBOX_3, RPGMENUITEMID - 7, "\c[Y5]", "\c[B1]", 192.1, 176.0);
					HudMessage(l:"UNITY", s:":"; HUDMSG_PLAIN, RPGMENUITEMID - 17, CR_GOLD, 192.1, 192.0, 0.0);

					HudMessage(d:0; HUDMSG_PLAIN, RPGMENUITEMID - 8, CR_WHITE, 256.1, 144.0, 0.0);
					HudMessage(d:0; HUDMSG_PLAIN, RPGMENUITEMID - 9, CR_WHITE, 256.1, 160.0, 0.0);
					HudMessage(d:0; HUDMSG_PLAIN, RPGMENUITEMID - 10, CR_WHITE, 256.1, 176.0, 0.0);
					HudMessage(d:GetUnity(); HUDMSG_PLAIN, RPGMENUITEMID - 16, CR_WHITE, 256.1, 192.0, 0.0);

					// draw bonus stat acquired
					HudMessage(s:"(\cd", d:0, s:"\c-)"; HUDMSG_PLAIN, RPGMENUITEMID - 11, CR_WHITE, 320.1, 144.0, 0.0);
					HudMessage(s:"(\cd", d:0, s:"\c-)"; HUDMSG_PLAIN, RPGMENUITEMID - 12, CR_WHITE, 320.1, 160.0, 0.0);
					HudMessage(s:"(\cd", d:0, s:"\c-)"; HUDMSG_PLAIN, RPGMENUITEMID - 13, CR_WHITE, 320.1, 176.0, 0.0);
				}

				HudMessage(s:"\c[Y5]Loot Penalty: \c-", d:PlayerActivities[pnum].vote_skips; HUDMSG_PLAIN, RPGMENUITEMID - 15, CR_WHITE, 192.1, 208.1, 0.0, 0.0);
				...
				DrawBoxText("=>", DND_NOLOOKUP, boxid, MBOX_4, RPGMENUPAGEID, 436.1, 44.0, "\c[B1]", "\c[Y5]");
			}
				...
				HudMessage(s:"--- ", l:"DND_MENU_GAIN_MISC", s:" ---"; HUDMSG_PLAIN, RPGMENUHELPID, CR_CYAN, 316.4, 44.0, 0.0, 0.0);
				
				DrawPlayerStats(pnum, DRAW_STAT_MISC);
				...
				DrawBoxText("DND_MENU_WEAPONDISCARD", DND_LANGUAGE_LOOKUP, boxid, MBOX_5, RPGMENUITEMID - 5, 192.1, 160.0, "\c[B1]", "\c[Y5]");
				DrawBoxText("DND_MENU_CRAFTING", DND_LANGUAGE_LOOKUP, boxid, MBOX_6, RPGMENUITEMID - 6, 192.1, 176.0, "\c[B1]", "\c[Y5]");
				DrawBoxText("DND_MENU_ACCESSORIES", DND_LANGUAGE_LOOKUP, boxid, MBOX_7, RPGMENUITEMID - 7, 192.1, 192.0, "\c[B1]", "\c[Y5]");
				DrawBoxText("DND_MENU_TRADING", DND_LANGUAGE_LOOKUP, boxid, MBOX_8, RPGMENUITEMID - 8, 192.1, 208.0, "\c[B1]", "\c[Y5]");
				DrawBoxText("DND_MENU_DUNGEONS", DND_LANGUAGE_LOOKUP, boxid, MBOX_9, RPGMENUITEMID - 9, 192.1, 224.0, "\c[B1]", "\c[Y5]");
			}
				...
				HudMessage(s:"\c[Y5]", l:"DND_MENU_EQUIPLEFT"; HUDMSG_PLAIN, RPGMENUITEMID - 2 * (INV_ICON_INDEX + 2), CR_CYAN, 236.4, 32.0, 0.0, 0.0);
				HudMessage(s:"\c[Y5]", l:"DND_MENU_EQUIPRIGHT"; HUDMSG_PLAIN, RPGMENUITEMID - 2 * (INV_ICON_INDEX + 2) - 1, CR_CYAN, 396.4, 32.0, 0.0, 0.0);
			}
			else if(curopt == MENU_LOAD_WEAPONDISCARD) {
				HudMessage(s:"--- ", l:"DND_MENU_HEAD_DISCARD", s:" ---"; HUDMSG_PLAIN, RPGMENUHELPID, CR_CYAN, 316.4, 44.0, 0.0, 0.0);
				
				for(i = 1; i < 9; ++i) {
					if(HasPlayerDiscardedSlot(pnum, i - 1)) {
						DrawBoxText(
							StrParam(s:"DND_MENU_ENABLE_WEP", d:i), 
							DND_LANGUAGE_LOOKUP, boxid, MBOX_1 + i - 1, RPGMENUITEMID - 1 - i, 192.1, 64.0 + i * 16.0, "\c[B1]", "\c[Y5]"
						);
					}
					else {
						DrawBoxText(
							StrParam(s:"DND_MENU_DISCARD_WEP", d:i), 
							DND_LANGUAGE_LOOKUP, boxid, MBOX_1 + i - 1, RPGMENUITEMID - 1 - i, 192.1, 64.0 + i * 16.0, "\c[B1]", "\c[Y5]"
						);
					}
				}

				// slot 9 has a different thing
				i = GetPlayerPreferredTempWeapon(pnum);
				if(i >= 0) {
					DrawBoxText(
						StrParam(l:"DND_MENU_TEMP_PREF", l:GetWeaponTag(i + FIRST_SLOT9_WEAPON)), 
						DND_NOLOOKUP, boxid, MBOX_1 + 8, RPGMENUITEMID - 10, 192.1, 64.0 + 9 * 16.0, "\c[B1]", "\c[Y5]"
					);
				}
				else if(i == -2) {
					DrawBoxText(
						"DND_MENU_TEMP_PREF_ANY", 
						DND_LANGUAGE_LOOKUP, boxid, MBOX_1 + 8, RPGMENUITEMID - 10, 192.1, 64.0 + 9 * 16.0, "\c[B1]", "\c[Y5]"
					);
				}
				else {
					DrawBoxText(
						"DND_MENU_TEMP_PREF_NONE", 
						DND_LANGUAGE_LOOKUP, boxid, MBOX_1 + 8, RPGMENUITEMID - 10, 192.1, 64.0 + 9 * 16.0, "\c[B1]", "\c[Y5]"
					);
				}

				SetHudClipRect(184, 216, 256, 128, 256);
				HudMessage(s:"\c[Y5]*\c- ", l:"DND_MENU_DISCARD_EXPLANATION"; HUDMSG_PLAIN, RPGMENUITEMID - 11, CR_WHITE, 192.1, 248.0, 0.0, 0.0);
				SetHudClipRect(0, 0, 0, 0, 0);
			}
			...
			else if(curopt == MENU_LOAD_DUNGEONS) {
				HudMessage(s:"--- ", l:"DND_MENU_HEAD_DUNGEONS", s:" ---"; HUDMSG_PLAIN, RPGMENUHELPID, CR_CYAN, 316.4, 44.0, 0.0, 0.0);

				i = CheckInventory("DnD_InventoryView");
				DrawBoxText("<=", DND_NOLOOKUP, i ? -1 : boxid, MBOX_3, RPGMENUID - 3, 184.1, 44.0, "\c[B1]", "\c[Y5]");
				...
				if(k != -1) {
					...
					if(j)
						UpdateCursorHoverData(CheckInventory("DnD_SelectedDungeonBox") - 1, DND_SYNC_ITEMSOURCE_PLAYERINVENTORY, 0, pnum, 0, HUDMAX_X, HUDMAX_Y);
				}
				else {
					SetFont("ORBBAKQ");
					HudMessage(s:"A"; HUDMSG_PLAIN, RPGMENUITEMID - 2, CR_WHITE, 316.4, 80.0, 0.0, 0.0);
				...
				if(j)
					...
					SetFont("SCHNOR");
				HudMessage(s:"A"; HUDMSG_PLAIN, RPGMENUITEMID - 1, CR_WHITE, 316.4, 80.0, 0.0, 0.0);
				...
				HandleWeaponPageDraw(pnum, curopt, 0, 1, boxid, -32, CurrentPane.cursize, redraw);
			else if(curopt == MENU_SHOP_WEAPON2)
				HandleWeaponPageDraw(pnum, curopt, 0, 2, boxid, -16, CurrentPane.cursize, redraw);
			else if(curopt == MENU_SHOP_WEAPON3_1)
				HandleWeaponPageDraw(pnum, curopt, 1, 3, boxid, -32, CurrentPane.cursize, redraw);
			else if(curopt == MENU_SHOP_WEAPON3_2)
				HandleWeaponPageDraw(pnum, curopt, 2, 3, boxid, -32, CurrentPane.cursize, redraw);
			else if(curopt == MENU_SHOP_WEAPON3_3)
				HandleWeaponPageDraw(pnum, curopt, -3, 3, boxid, -32, CurrentPane.cursize, redraw);
			else if(curopt == MENU_SHOP_WEAPON4_1)
				HandleWeaponPageDraw(pnum, curopt, 1, 4, boxid, -24, CurrentPane.cursize, redraw);
			else if(curopt == MENU_SHOP_WEAPON4_2)
				HandleWeaponPageDraw(pnum, curopt, -2, 4, boxid, -32, CurrentPane.cursize, redraw);
			else if(curopt == MENU_SHOP_WEAPON5_1)
				HandleWeaponPageDraw(pnum, curopt, 1, 5, boxid, -32, CurrentPane.cursize, redraw);
			else if(curopt == MENU_SHOP_WEAPON5_2)
				HandleWeaponPageDraw(pnum, curopt, -2, 5, boxid, -24, CurrentPane.cursize, redraw);
			else if(curopt == MENU_SHOP_WEAPON6_1)
				HandleWeaponPageDraw(pnum, curopt, 1, 6, boxid, -40, CurrentPane.cursize, redraw);
			else if(curopt == MENU_SHOP_WEAPON6_2)
				HandleWeaponPageDraw(pnum, curopt, -2, 6, boxid, -16, CurrentPane.cursize, redraw);
			else if(curopt == MENU_SHOP_WEAPON7)
				HandleWeaponPageDraw(pnum, curopt, 0, 7, boxid, -16, CurrentPane.cursize, redraw);
			else if(curopt == MENU_SHOP_WEAPON8)
				HandleWeaponPageDraw(pnum, curopt, 0, 8, boxid, 0, CurrentPane.cursize, redraw);
				...
				HandleAmmoPageDraw(pnum, boxid, DND_AMMOSLOT_CLIP, 1, AMMOSLOT_CLIP_BEGIN, AMMOSLOT_CLIP_END, false);
			else if(curopt == MENU_SHOP_AMMO_1_2)
				HandleAmmoPageDraw(pnum, boxid, DND_AMMOSLOT_CLIP, -1, AMMOSLOT_CLIP2_BEGIN, AMMOSLOT_CLIP2_END, false);
			else if(curopt == MENU_SHOP_AMMO_2)
				HandleAmmoPageDraw(pnum, boxid, DND_AMMOSLOT_SHELL, 1, AMMOSLOT_SHELL_BEGIN, AMMOSLOT_SHELL_END, false);
			else if(curopt == MENU_SHOP_AMMO_2_2)
				HandleAmmoPageDraw(pnum, boxid, DND_AMMOSLOT_SHELL, -1, AMMOSLOT_SHELL2_BEGIN, AMMOSLOT_SHELL2_END, false);
			else if(curopt == MENU_SHOP_AMMO_3)
				HandleAmmoPageDraw(pnum, boxid, DND_AMMOSLOT_ROCKET, 0, AMMOSLOT_ROCKET_BEGIN, AMMOSLOT_ROCKET_END, false);
			else if(curopt == MENU_SHOP_AMMO_4_1)
				HandleAmmoPageDraw(pnum, boxid, DND_AMMOSLOT_CELL, 1, AMMOSLOT_CELL_BEGIN, AMMOSLOT_CELL_END, false);
			else if(curopt == MENU_SHOP_AMMO_4_2)
				...
				for(i  = 0; i < MAXACCOUNTITEMS; ++i)
					DrawToggledImage(pnum, SHOP_ACCOUNT_BEGIN + i, boxid, i, OBJ_RESEARCH | OBJ_ACCOUNT, CR_WHITE, CR_GREEN, GetItemName(SHOP_ACCOUNT_BEGIN + i), GetShopItemInfo(SHOP_ACCOUNT_BEGIN + i, DND_SHOPINFO_MAX), CR_RED);
			}
				...
				HudMessage(s:"\cv*\c- ", l:"DND_MENU_BOBBY1", s:"\n\n\cv*\c- ", l:"DND_MENU_BOBBY2", s:"\n\n\cv*\c- ", l:"DND_MENU_BOBBY3"; HUDMSG_PLAIN, RPGMENUITEMID - 2, CR_WHITE, 192.1, 152.1 + 4.0 * ScrollPos.x, 0.0, 0.0);
				SetHudClipRect(0, 0, 0, 0);
			}
				...
				HudMessage(s:"\c[Y5]______________________________________"; HUDMSG_PLAIN, RPGMENUITEMID - 14, CR_CYAN, 316.4, 192.1, 0.0, 0.0); 
				
				HudMessage(s:"\c[Y5]", l:"DND_MENU_AVGMONLEVEL", s:": \cj", d:CheckInventory("DnD_PTotalLevel") / PlayerCount(); HUDMSG_PLAIN, RPGMENUITEMID - 9, CR_WHITE, 192.1, 212.0, 0.0, 0.0);
				str mapinfo = "DND_MENU_DISABLED";
				if(CheckInventory("ShowMapInfo"))
					mapinfo = "DND_MENU_ENABLED";
				DrawBoxText(StrParam(l:"DND_MENU_MAPINFO", l:mapinfo), DND_NOLOOKUP, boxid, MBOX_10, RPGMENUITEMID - 10, 192.1, 228.0, "\c[B1]", "\c[Y5]");
				...
				if(GetCVar("dnd_detailedmods"))
					mapinfo = "DND_MENU_ENABLED";
				DrawBoxText(StrParam(l:"DND_MENU_MODTIERS", l:mapinfo), DND_NOLOOKUP, boxid, MBOX_11, RPGMENUITEMID - 11, 192.1, 244.0, "\c[B1]", "\c[Y5]");
				...
				if(GetCVar("dnd_ignoretempweps"))
					mapinfo = "DND_MENU_ENABLED";
				DrawBoxText(StrParam(l:"DND_MENU_IGNORETEMPS", l:mapinfo), DND_NOLOOKUP, boxid, MBOX_12, RPGMENUITEMID - 12, 192.1, 260.0, "\c[B1]", "\c[Y5]");
				if(CheckInventory("SetTempWepIgnore"))
					SetCVar("dnd_ignoretempweps", !GetCVar("dnd_ignoretempweps"));

				i = GetCVar("dnd_pickup_filter");
				mapinfo = StrParam(s:"DND_MENU_FILTER_", d:i);
				DrawBoxText(StrParam(l:"DND_MENU_AUTOPICK_FILTER", l:mapinfo), DND_NOLOOKUP, boxid, MBOX_13, RPGMENUITEMID - 13, 192.1, 276.0, "\c[B1]", "\c[Y5]");
			}
				...
				HudMessage(l:"DND_CLASS", s:": \cj", l:GetClassLabel(j, DND_CLASS_LABEL_NAME); HUDMSG_PLAIN, RPGMENUHELPID - 10, CR_CYAN, 316.4, 52.1, 0.0, 0.0);
				
				SetHudClipRect(192, 64, 256, 216, 256);
				toShow = GetClassPerkDisplay(j, true);
				HudMessage(s:toshow; HUDMSG_PLAIN, RPGMENUITEMID - 2, CR_WHITE, 192.1, 88.1 + 4.0 * ScrollPos.x, 0.0, 0.0);
				SetHudClipRect(0, 0, 0, 0);
			}
				...
				HudMessage(l:"DND_MENU_ENERGYTEXT"; HUDMSG_PLAIN, RPGMENUITEMID - 4, CR_WHITE, 192.1, 304.1 + 4.0 * ScrollPos.x, 0.0, 0.0);
				HudMessage(l:"DND_MENU_FIRETEXT"; HUDMSG_PLAIN, RPGMENUITEMID - 5, CR_WHITE, 192.1, 384.1 + 4.0 * ScrollPos.x, 0.0, 0.0);
				HudMessage(l:"DND_MENU_ICETEXT"; HUDMSG_PLAIN, RPGMENUITEMID - 6, CR_WHITE, 192.1, 464.1 + 4.0 * ScrollPos.x, 0.0, 0.0);
				HudMessage(l:"DND_MENU_POISONTEXT"; HUDMSG_PLAIN, RPGMENUITEMID - 7, CR_WHITE, 192.1, 544.1 + 4.0 * ScrollPos.x, 0.0, 0.0);
				HudMessage(l:"DND_MENU_LIGHTNINGTEXT"; HUDMSG_PLAIN, RPGMENUITEMID - 8, CR_WHITE, 192.1, 624.1 + 4.0 * ScrollPos.x, 0.0, 0.0);
				...
				HudMessage(l:"DND_NFT_INTRO"; HUDMSG_PLAIN, RPGMENUITEMID - 1, CR_WHITE, 316.4, 72.1, 0.0, 0.0);
			...
			if(curopt < MENU_LOAD_CRAFTING || curopt > MENU_LOAD_CRAFTING_TRANSMUTING) {
				if(mainboxid == MAINBOX_LOAD)
					...
					HudMessage(s:"\c[Y5]", l:"DND_MENU_SIDE_LOADOUT"; HUDMSG_PLAIN, RPGMENULISTID - 2, -1, 96.0, 204.0, 0.0);
				
				if(mainboxid == MAINBOX_SHOP)
					HudMessage(s:"\c[B1]", l:"DND_MENU_HEAD_SHOP"; HUDMSG_PLAIN, RPGMENULISTID - 3, -1, 96.0, 221.0, 0.0);
				else
					HudMessage(s:"\c[Y5]", l:"DND_MENU_HEAD_SHOP"; HUDMSG_PLAIN, RPGMENULISTID - 3, -1, 96.0, 221.0, 0.0);
				
				if(mainboxid == MAINBOX_RESEARCH)
					HudMessage(s:"\c[B1]", l:"DND_MENU_RESEARCH"; HUDMSG_PLAIN, RPGMENULISTID - 6, -1, 97.0, 251.0, 0.0);
				else
				...
				if(mainboxid == MAINBOX_HELP)
					...
					HudMessage(s:"\c[Y5]", l:"DND_MENU_HELP"; HUDMSG_PLAIN, RPGMENULISTID - 5, -1, 96.0, 287.0, 0.0);
				
				// hoping nobody lives more than 9999 maps --- find an adjustment for later ahh yes
				temp = CheckInventory("RoundsSurvived");
				if(temp <= 999)
					HudMessage(s:"\c[Y5]", l:"DND_MENU_LASTED", s:": \cj", d:temp; HUDMSG_PLAIN, RPGMENUNAMEID, -1, 60.1, 140.0, 0.0);
				else
					HudMessage(s:"\c[Y5]", l:"DND_MENU_LASTED", s:":\cj", d:temp; HUDMSG_PLAIN, RPGMENUNAMEID, -1, 60.1, 140.0, 0.0);
			}
			...
			if(curopt == MENU_STAT1) { // stat screen
				...
				if(HasLeftClicked(pnum)) {
					if(boxid > 0) {
						if(boxid == MBOX_9) {
							ResetDungeonSelectData(pnum);
							UpdateMenuPosition(MENU_LOAD_DUNGEONS);
						}
				...
				if(HasPlayerClicked(pnum) && boxid != MAINBOX_NONE) {
					temp = boxid;
					if(temp < 9) {
						if(HasPlayerDiscardedSlot(pnum, temp - 1))
							temp = -temp;
						UpdateActivity(pnum, DND_ACTIVITY_WEAPONDISCARD, temp, 0);
					}
					else {
						UpdateActivity(pnum, DND_ACTIVITY_WEAPONDISCARD, temp, 2);
					}

					if(temp > 0) {
						switch(temp) {
							case 1:
								TakeInventory(Weapons_Data[DND_WEAPON_CHAINSAW].name, 1);
								if(CheckWeapon(Weapons_Data[DND_WEAPON_CHAINSAW].name))
									TakeInventory("H_WeaponSlot1", 1);
							break;
							case 2:
								TakeInventory(Weapons_Data[DND_WEAPON_PISTOL].name, 1);
								if(CheckWeapon(Weapons_Data[DND_WEAPON_PISTOL].name))
									TakeInventory("H_WeaponSlot2", 1);
							break;
							case 3:
								TakeInventory(Weapons_Data[DND_WEAPON_SHOTGUN].name, 1);
								if(CheckWeapon(Weapons_Data[DND_WEAPON_SHOTGUN].name))
									TakeInventory("H_WeaponSlot3", 1);
							break;
							case 4:
								TakeInventory(Weapons_Data[DND_WEAPON_SUPERSHOTGUN].name, 1);
								if(CheckWeapon(Weapons_Data[DND_WEAPON_SUPERSHOTGUN].name))
									TakeInventory("H_WeaponSlot3X", 1);
							break;
							case 5:
								TakeInventory(Weapons_Data[DND_WEAPON_MACHINEGUN].name, 1);
								if(CheckWeapon(Weapons_Data[DND_WEAPON_MACHINEGUN].name))
									TakeInventory("H_WeaponSlot4", 1);
							break;
							case 6:
								TakeInventory(Weapons_Data[DND_WEAPON_ROCKETLAUNCHER].name, 1);
								if(CheckWeapon(Weapons_Data[DND_WEAPON_ROCKETLAUNCHER].name))
									TakeInventory("H_WeaponSlot5", 1);
							break;
							case 7:
								TakeInventory(Weapons_Data[DND_WEAPON_PLASMARIFLE].name, 1);
								if(CheckWeapon(Weapons_Data[DND_WEAPON_PLASMARIFLE].name))
									TakeInventory("H_WeaponSlot6", 1);
							break;
							case 8:
								TakeInventory(Weapons_Data[DND_WEAPON_BFG6000].name, 1);
								if(CheckWeapon(Weapons_Data[DND_WEAPON_BFG6000].name))
				...
				if(HasLeftClicked(pnum)) {
					if(boxid != MAINBOX_NONE) {
						if(boxid == MBOX_6 && curopt != MENU_LAST_ACCESSORY_PAGE)
							...
							// I don't have this
							if(!CheckInventory(GetAccessoryName(temp)))
				...
				if(HasLeftClicked(pnum)) {
					if(!CheckInventory("DnD_InventoryView")) {
						if(boxid == MBOX_1) {
							SetInventory("DnD_Inventory_Filter", DND_BOXFILTER_DUNGEONS);
							GiveInventory("DnD_InventoryView", 1);
							LocalAmbientSound("RPG/MenuChoose", 127);
						}
						else if(boxid == MBOX_2) {
							// allow to open portal to dungeon
							temp = CheckInventory("DnD_SelectedDungeonBox");
							if(!temp)
								ShowPopup(POPUP_NODUNGEONSELECTED, false, 0);
							else if(DungeonInformation.dungeon_id == -1) {
								ACS_NamedExecuteWithResult("DnD Dungeon Portal Create", pnum, temp - 1, GetSelectedDungeonData(pnum).id);
								ResetDungeonSelectData(pnum);
								GiveInventory("DnD_RefreshPane", 1);
								LocalAmbientSound("RPG/MenuChoose", 127);
							}
							else
								ShowPopup(POPUP_ALREADYDUNGEONACTIVE, false, 0);
						}
						else if(boxid == MBOX_3) {
							SetInventory("DnD_Inventory_Filter", 0);
							TakeInventory("DnD_InventoryView", 1);
							GiveInventory("DnD_CleanInventoryRequest", 1);
							UpdateMenuPosition(MENU_LOAD);
						}
						...
						// inventory view clicks will attempt to place a proper (dungeon key) item to the slot
						HandleDungeonKeySelection(pnum, boxid, temp);
					}
				}
				else if(HasRightClicked(pnum) && boxid == MBOX_1 && CheckInventory("DnD_SelectedDungeonBox")) {
					ResetSelectedDungeonInformation(pnum);
					LocalAmbientSound("RPG/MenuChoose", 127);
				}
				else if(HasPressedLeft(pnum)) {
				...
				if(HasLeftClicked(pnum)) {
					switch(boxid) {
						...
						break;
						case 4:
							UpdateMenuPosition(MENU_SHOP_AMMO_4_1);
						break;
						case 5:
				...
				HandleAmmoPageInput(pnum, DND_AMMOSLOT_CELL, boxid, AMMOSLOT_CELL_BEGIN, -1, MENU_SHOP_AMMO_4_2, false);
			else if(curopt == MENU_SHOP_AMMO_4_2)
				...
				if(HasLeftClicked(pnum)) {
					if(boxid == MBOX_1)
						...
						if(GetPlayerCredit(pnum) >= DND_BUDGET_CONVERSION_CREDIT) {
							TakeCredit(DND_BUDGET_CONVERSION_CREDIT);
					...
					// donate budget option, basically credit to budget conversion
					// 10,000 credits => 1k budget
					if(GetPlayerCredit(pnum) >= DND_BUDGET_CONVERSION_CREDIT * 10) {
						TakeCredit(DND_BUDGET_CONVERSION_CREDIT * 10);
				...
				if(HasLeftClicked(pnum)) {
					...
					else if(boxid == MBOX_13) {
						ACS_NamedExecuteWithResult("DnD Filter Click", pnum, 0, 0, 0);
						LocalAmbientSound("RPG/MenuChoose", 127);
						temp = -1;
					}
					if(boxid != MAINBOX_NONE && temp != -1)
				...
				if(HasLeftClicked(pnum) && boxid != MAINBOX_NONE) {
					...
					if(GetPlayerCredit(pnum) >= NFT_COST) {
...
Script "DnD Open Stash" (void) NET {
	ACS_ExecuteAlways(900, 0, MENU_LOAD_STASH, 2);
}

Script "DnD Dump to Stash" (int stackableOnly) NET {
...
Script "DnD Filter Click" (int pnum) CLIENTSIDE {
	if(ConsolePlayerNumber() != pnum)
		Terminate;
	int f = GetCVar("dnd_pickup_filter");
	++f;
	if(f > 7)
		f = 0;
	SetCVar("dnd_pickup_filter", f);
	NamedRequestScriptPuke("DND Set Pickup Filter", pnum, f);
	SetResultValue(0);
}

Script "DND Set Pickup Filter" (int pnum, int val) NET {
	SetActorInventory(pnum + P_TIDSTART, "PlayerPickupFilter", val);
}

Script "DnD Setup Menu Vars" (void) {
	GetShopItemInfo(0, 0);
	SetupComplete(SETUP_STATE1, SETUP_ITEMTABLES);
}

Script "DnD Setup Menu Vars - CS" (void) CLIENTSIDE {
	GetShopItemInfo(0, 0);
	SetupComplete(SETUP_STATE1, SETUP_ITEMTABLES);
}
