...
int CreateProjectile(int owner, int p_helper_tid, str projectile, int angle, int pitch, int spd, Vec3_T* velocity, Vec3_T* vPos, int flags = 0, int extra = 0, int extra2 = 0, int dmg_category = 0) {
	...
	if(isPlayer(owner) && (g = PlayerModData[owner - P_TIDSTART].vals[PSTAT_PROJSPEED])) {
		g += 1.0;
		if(g > 3.0)
			g = 3.0;
...
void HandleAttackEvent(int wepid, int isSpecial, int extra) {
	...
	if(PlayerModData[pnum].vals[PSTAT_EX_CHANCE_CASTELEMSPELLONATK] >= random(1, 100) && !CheckInventory("RandomElementalSpellCooldown")) {
