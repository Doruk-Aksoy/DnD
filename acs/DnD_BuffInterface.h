...
void HandleGenericFade(int player_tid, int origin_tid, int r, int g, int b, int intensity, int duration, int effect_factor, int rank = DND_VISIONRANK_CURSE) {
    ...
    int temp = PlayerModData[player_tid - P_TIDSTART].vals[PSTAT_IMP_REDUCEDVISIONIMPAIR];
    if(temp)
...
Script "DnD Give Buff" (int debuff_id, int debuff_flags) {
	...
	if(!CheckUniquePropertyOnPlayer(pnum, PUP_CURSEIMMUNITY) || !i) {
        ...
        if(i) {
            ...
            curse_effect = curse_effect * (100 - PlayerModData[pnum].vals[PSTAT_REDUCEDCURSEEFFECT]) / 100;
            if(curse_effect <= 0) {
                SetResultValue(0);
                Terminate;
            }
